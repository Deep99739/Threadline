...
def main() -> None:
    ...
    adrs = sorted((ROOT / "docs" / "adr").glob("ADR-00[1-7]-*.md"))
    if len(adrs) != 7:
        raise SystemExit(f"Expected seven current ADRs, found {len(adrs)}")
    ...
    print(f"Foundation structure passes: 7 ADRs and {len(cases)} eval cases.")
