...
def main() -> None:
    ...
    adrs = sorted((ROOT / "docs" / "adr").glob("ADR-00[1-8]-*.md"))
    if len(adrs) != 8:
        raise SystemExit(f"Expected eight current ADRs, found {len(adrs)}")
    ...
    print(f"Foundation structure passes: 8 ADRs and {len(cases)} eval cases.")
