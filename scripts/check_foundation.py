...
def main() -> None:
    ...
    adrs = sorted((ROOT / "docs" / "adr").glob("ADR-00[1-6]-*.md"))
    if len(adrs) != 6:
        raise SystemExit(f"Expected six foundation ADRs, found {len(adrs)}")
    ...
    print(f"Foundation structure passes: 6 ADRs and {len(cases)} eval cases.")
