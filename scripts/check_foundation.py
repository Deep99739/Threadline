...
def main() -> None:
    ...
    adrs = sorted((ROOT / "docs" / "adr").glob("ADR-00[1-9]-*.md"))
    if len(adrs) != 9:
        raise SystemExit(f"Expected nine current ADRs, found {len(adrs)}")
    ...
    print(f"Foundation structure passes: 9 ADRs and {len(cases)} eval cases.")
